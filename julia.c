...
/*   Updated: 2023/09/27 15:01:05 by shamzaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void julia_draw(t_graphics *graph)
{
    ...
    while (++p.y < HEIGHT)
    {
        ...
        while(++p.x < WIDTH)
        {
            p.real = p.x_min + ((p.x - graph->x) * (p.x_max - p.x_min) / WIDTH + (double)graph->x / WIDTH * (p.x_max - p.x_min));
            p.imag = p.y_min + ((p.y - graph->y) * (p.y_max - p.y_min) / HEIGHT + (double)graph->y / HEIGHT * (p.y_max - p.y_min));
